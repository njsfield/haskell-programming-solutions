...
gimmePerson = do
  ...
  case mkPerson name (read age) of
    Right (Person n a) -> putStrLn $ "Yay! Successfully go a person: " ++ name ++ ", " ++ age
    Left NameEmpty -> putStrLn "Invalid! Name is empty"
    Left AgeTooLow -> putStrLn $ "Invalid! " ++ age ++ " is too low"
    Left (PersonInvalidUnknown e) -> putStrLn $ "Invalid! " ++ e
