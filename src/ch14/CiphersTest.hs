...
run_caesar = do
  ...
  quickCheck $ forAll caesarGen (\(x, y) -> (uncaesar x (caesar x y)) == y)

main :: IO ()
main =
  hspec $ do
    describe "Cipher Tests" $ do
      it "Ceasar Tests" $ do run_caesar
      it "Vigenere Tests" $ do vigenere "MEET AT DAWN" "ALLY" `shouldBe` "MPPR AE OYWY"
